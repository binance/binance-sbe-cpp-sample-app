...
#ifndef _SPOT_SBE_EXTERNALLOCKUPDATEEVENT_CXX_H_
#define _SPOT_SBE_EXTERNALLOCKUPDATEEVENT_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(27);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(604);
    ...
    ExternalLockUpdateEvent() = default;
    ...
    ExternalLockUpdateEvent(char *buffer, const std::uint64_t bufferLength) :
        ExternalLockUpdateEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ExternalLockUpdateEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ExternalLockUpdateEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(604);
    }
    ...
    ExternalLockUpdateEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ExternalLockUpdateEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ExternalLockUpdateEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    ExternalLockUpdateEvent &sbeRewind()
    {
    ...
    {
        ExternalLockUpdateEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ExternalLockUpdateEvent &eventTime(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *clearTimeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t clearTimeId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t clearTimeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool clearTimeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t clearTimeEncodingOffset() SBE_NOEXCEPT
    {
        return 8;
    }

    static SBE_CONSTEXPR std::int64_t clearTimeNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t clearTimeMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t clearTimeMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t clearTimeEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t clearTime() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ExternalLockUpdateEvent &clearTime(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ExternalLockUpdateEvent &qtyExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *lockedQtyDeltaMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t lockedQtyDeltaId() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t lockedQtyDeltaSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool lockedQtyDeltaInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t lockedQtyDeltaEncodingOffset() SBE_NOEXCEPT
    {
        return 17;
    }

    static SBE_CONSTEXPR std::int64_t lockedQtyDeltaNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t lockedQtyDeltaMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t lockedQtyDeltaMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t lockedQtyDeltaEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t lockedQtyDelta() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ExternalLockUpdateEvent &lockedQtyDelta(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD std::uint16_t subscriptionId() const SBE_NOEXCEPT
    {
        ...
        std::uint16_t val;
        std::memcpy(&val, m_buffer + m_offset + 25, sizeof(std::uint16_t));
        return SBE_LITTLE_ENDIAN_ENCODE_16(val);
    }

    ExternalLockUpdateEvent &subscriptionId(const std::uint16_t value) SBE_NOEXCEPT
    {
        std::uint16_t val = SBE_LITTLE_ENDIAN_ENCODE_16(value);
        std::memcpy(m_buffer + m_offset + 25, &val, sizeof(std::uint16_t));
        return *this;
    }
    ...
    ExternalLockUpdateEvent &putAsset(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ExternalLockUpdateEvent &putAsset(const std::string &str)
    {
        if (str.length() > 254)
        {
    ...
    #if __cplusplus >= 201703L
    ExternalLockUpdateEvent &putAsset(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ExternalLockUpdateEvent &_writer)
{
    ExternalLockUpdateEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ExternalLockUpdateEvent", )";
    ...
    builder << ", ";
    builder << R"("lockedQtyDelta": )";
    builder << +writer.lockedQtyDelta();
