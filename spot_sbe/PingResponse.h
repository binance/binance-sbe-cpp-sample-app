...
#ifndef _SPOT_SBE_PINGRESPONSE_CXX_H_
#define _SPOT_SBE_PINGRESPONSE_CXX_H_
...
public:
    ...
    PingResponse(char *buffer, const std::uint64_t bufferLength) :
        PingResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    PingResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        PingResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    PingResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PingResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PingResponse &_writer)
{
    PingResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
