...
#ifndef _SPOT_SBE_ICEBERGPARTSFILTER_CXX_H_
#define _SPOT_SBE_ICEBERGPARTSFILTER_CXX_H_
...
public:
    ...
    IcebergPartsFilter(char *buffer, const std::uint64_t bufferLength) :
        IcebergPartsFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    IcebergPartsFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        IcebergPartsFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    IcebergPartsFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    IcebergPartsFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    IcebergPartsFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        IcebergPartsFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::IcebergParts;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::IcebergParts);
    }

    SBE_NODISCARD static const char *filterLimitMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t filterLimitId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t filterLimitSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool filterLimitInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t filterLimitEncodingOffset() SBE_NOEXCEPT
    {
        return 0;
    }

    static SBE_CONSTEXPR std::int64_t filterLimitNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t filterLimitMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t filterLimitMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t filterLimitEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t filterLimit() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    IcebergPartsFilter &filterLimit(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const IcebergPartsFilter &_writer)
{
    IcebergPartsFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "IcebergPartsFilter", )";
    ...
    builder << ", ";
    builder << R"("filterLimit": )";
    builder << +writer.filterLimit();
