...
public:
    ...
    {
        WebSocketSessionLogonResponse skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    WebSocketSessionLogonResponse &putApiKey(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    WebSocketSessionLogonResponse &putApiKey(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #if __cplusplus >= 201703L
    WebSocketSessionLogonResponse &putApiKey(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
