...
#ifndef _SPOT_SBE_WEBSOCKETSESSIONLOGONRESPONSE_CXX_H_
#define _SPOT_SBE_WEBSOCKETSESSIONLOGONRESPONSE_CXX_H_
...
public:
    ...
    WebSocketSessionLogonResponse() = default;
    ...
    WebSocketSessionLogonResponse(char *buffer, const std::uint64_t bufferLength) :
        WebSocketSessionLogonResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    WebSocketSessionLogonResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        WebSocketSessionLogonResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    WebSocketSessionLogonResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    WebSocketSessionLogonResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    WebSocketSessionLogonResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    WebSocketSessionLogonResponse &sbeRewind()
    {
    ...
    {
        WebSocketSessionLogonResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    WebSocketSessionLogonResponse &authorizedSince(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    WebSocketSessionLogonResponse &connectedSince(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    WebSocketSessionLogonResponse &returnRateLimits(const BoolEnum::Value value) SBE_NOEXCEPT
    {
    ...
    WebSocketSessionLogonResponse &serverTime(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    WebSocketSessionLogonResponse &userDataStream(const BoolEnum::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    WebSocketSessionLogonResponse &putLoggedOnApiKey(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    WebSocketSessionLogonResponse &putLoggedOnApiKey(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #if __cplusplus >= 201703L
    WebSocketSessionLogonResponse &putLoggedOnApiKey(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const WebSocketSessionLogonResponse &_writer)
{
    WebSocketSessionLogonResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "WebSocketSessionLogonResponse", )";
