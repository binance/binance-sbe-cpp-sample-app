...
#ifndef _SPOT_SBE_GROUPSIZE16ENCODING_CXX_H_
#define _SPOT_SBE_GROUPSIZE16ENCODING_CXX_H_
...
public:
    ...
    GroupSize16Encoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        GroupSize16Encoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    GroupSize16Encoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        GroupSize16Encoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    GroupSize16Encoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    GroupSize16Encoding &blockLength(const std::uint16_t value) SBE_NOEXCEPT
    {
    ...
    GroupSize16Encoding &numInGroup(const std::uint16_t value) SBE_NOEXCEPT
    {
        std::uint16_t val = SBE_LITTLE_ENDIAN_ENCODE_16(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, GroupSize16Encoding &writer)
{
    builder << '{';
