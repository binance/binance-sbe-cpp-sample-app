...
#ifndef _SPOT_SBE_REFERENCEPRICERESPONSE_CXX_H_
#define _SPOT_SBE_REFERENCEPRICERESPONSE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(217);
    ...
    ReferencePriceResponse() = default;
    ...
    ReferencePriceResponse(char *buffer, const std::uint64_t bufferLength) :
        ReferencePriceResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ReferencePriceResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ReferencePriceResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(217);
    }
    ...
    ReferencePriceResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ReferencePriceResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ReferencePriceResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    ReferencePriceResponse &sbeRewind()
    {
    ...
    {
        ReferencePriceResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ReferencePriceResponse &priceExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *referencePriceMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t referencePriceId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t referencePriceSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool referencePriceInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t referencePriceEncodingOffset() SBE_NOEXCEPT
    {
        return 1;
    }

    static SBE_CONSTEXPR std::int64_t referencePriceNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t referencePriceMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t referencePriceMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t referencePriceEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t referencePrice() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ReferencePriceResponse &referencePrice(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ReferencePriceResponse &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ReferencePriceResponse &putSymbol(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ReferencePriceResponse &putSymbol(const std::string &str)
    {
        if (str.length() > 254)
        {
    ...
    #if __cplusplus >= 201703L
    ReferencePriceResponse &putSymbol(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ReferencePriceResponse &_writer)
{
    ReferencePriceResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ReferencePriceResponse", )";
    ...
    builder << ", ";
    builder << R"("referencePrice": )";
    builder << +writer.referencePrice();
