...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(312);
    ...
    {
        CancelOrderListResponse skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
public:
    ...
    #pragma GCC diagnostic pop
            ...
            m_bufferLength = bufferLength;
            GroupSize16Encoding dimensions(buffer, *pos, bufferLength, actingVersion);
            dimensions.blockLength(static_cast<std::uint16_t>(116));
            dimensions.numInGroup(static_cast<std::uint16_t>(count));
            m_index = 0;
            m_count = count;
            m_blockLength = 116;
