...
#ifndef _SPOT_SBE_MAXASSETFILTER_CXX_H_
#define _SPOT_SBE_MAXASSETFILTER_CXX_H_
...
public:
    ...
    MaxAssetFilter(char *buffer, const std::uint64_t bufferLength) :
        MaxAssetFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MaxAssetFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MaxAssetFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MaxAssetFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MaxAssetFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MaxAssetFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::MaxAsset;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::MaxAsset);
    }
    ...
    MaxAssetFilter &qtyExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    MaxAssetFilter &maxQty(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    MaxAssetFilter &putAsset(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    MaxAssetFilter &putAsset(const std::string &str)
    {
        if (str.length() > 254)
        {
    ...
    #if __cplusplus >= 201703L
    MaxAssetFilter &putAsset(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MaxAssetFilter &_writer)
{
    MaxAssetFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
