...
#ifndef _SPOT_SBE_MAXNUMORDERSFILTER_CXX_H_
#define _SPOT_SBE_MAXNUMORDERSFILTER_CXX_H_
...
public:
    ...
    MaxNumOrdersFilter(char *buffer, const std::uint64_t bufferLength) :
        MaxNumOrdersFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MaxNumOrdersFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MaxNumOrdersFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MaxNumOrdersFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MaxNumOrdersFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MaxNumOrdersFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        MaxNumOrdersFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::MaxNumOrders;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::MaxNumOrders);
    }
    ...
    MaxNumOrdersFilter &maxNumOrders(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MaxNumOrdersFilter &_writer)
{
    MaxNumOrdersFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MaxNumOrdersFilter", )";
