...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(207);
    ...
    {
        Ticker24hSymbolMiniResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Ticker24hSymbolMiniResponse &putVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    Ticker24hSymbolMiniResponse &putVolume(std::span<const std::uint8_t> src)
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Ticker24hSymbolMiniResponse &putVolume(T&& src)  SBE_NOEXCEPT requires
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Ticker24hSymbolMiniResponse &putQuoteVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    Ticker24hSymbolMiniResponse &putQuoteVolume(std::span<const std::uint8_t> src)
    {
        const std::size_t srcLength = src.size();
        if (srcLength > 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Ticker24hSymbolMiniResponse &putQuoteVolume(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
