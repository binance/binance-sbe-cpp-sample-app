...
#ifndef _SPOT_SBE_EXCHANGEMAXNUMORDERLISTSFILTER_CXX_H_
#define _SPOT_SBE_EXCHANGEMAXNUMORDERLISTSFILTER_CXX_H_
...
public:
    ...
    ExchangeMaxNumOrderListsFilter() = default;
    ...
    ExchangeMaxNumOrderListsFilter(char *buffer, const std::uint64_t bufferLength) :
        ExchangeMaxNumOrderListsFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ExchangeMaxNumOrderListsFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ExchangeMaxNumOrderListsFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ExchangeMaxNumOrderListsFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ExchangeMaxNumOrderListsFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ExchangeMaxNumOrderListsFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    ExchangeMaxNumOrderListsFilter &sbeRewind()
    {
    ...
    {
        ExchangeMaxNumOrderListsFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::ExchangeMaxNumOrderLists;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::ExchangeMaxNumOrderLists);
    }

    SBE_NODISCARD static const char *maxNumOrderListsMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t maxNumOrderListsId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t maxNumOrderListsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool maxNumOrderListsInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t maxNumOrderListsEncodingOffset() SBE_NOEXCEPT
    {
        return 0;
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrderListsNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrderListsMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrderListsMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t maxNumOrderListsEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t maxNumOrderLists() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ExchangeMaxNumOrderListsFilter &maxNumOrderLists(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ExchangeMaxNumOrderListsFilter &_writer)
{
    ExchangeMaxNumOrderListsFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ExchangeMaxNumOrderListsFilter", )";
    ...
    builder << ", ";
    builder << R"("maxNumOrderLists": )";
    builder << +writer.maxNumOrderLists();
