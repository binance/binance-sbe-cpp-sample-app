...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(182);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(205);
    ...
    {
        Ticker24hSymbolFullResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Ticker24hSymbolFullResponse &putLastQty(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    Ticker24hSymbolFullResponse &putLastQty(std::span<const std::uint8_t> src)
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Ticker24hSymbolFullResponse &putLastQty(T&& src)  SBE_NOEXCEPT requires
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Ticker24hSymbolFullResponse &putVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    Ticker24hSymbolFullResponse &putVolume(std::span<const std::uint8_t> src)
    {
        const std::size_t srcLength = src.size();
        if (srcLength > 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Ticker24hSymbolFullResponse &putVolume(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Ticker24hSymbolFullResponse &putQuoteVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    Ticker24hSymbolFullResponse &putQuoteVolume(std::span<const std::uint8_t> src)
    {
        const std::size_t srcLength = src.size();
        if (srcLength > 16)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Ticker24hSymbolFullResponse &putQuoteVolume(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
