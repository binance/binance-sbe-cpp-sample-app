...
#ifndef _SPOT_SBE_OPTIONALVARSTRING8_CXX_H_
#define _SPOT_SBE_OPTIONALVARSTRING8_CXX_H_
...
public:
    ...
    OptionalVarString8(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        OptionalVarString8(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    OptionalVarString8(
        char *buffer,
        const std::uint64_t bufferLength) :
        OptionalVarString8(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    OptionalVarString8 &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    OptionalVarString8 &length(const std::uint8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, OptionalVarString8 &writer)
{
    builder << '{';
    builder << R"("length": )";
