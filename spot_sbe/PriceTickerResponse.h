...
#ifndef _SPOT_SBE_PRICETICKERRESPONSE_CXX_H_
#define _SPOT_SBE_PRICETICKERRESPONSE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(210);
    ...
    PriceTickerResponse(char *buffer, const std::uint64_t bufferLength) :
        PriceTickerResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    PriceTickerResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        PriceTickerResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    PriceTickerResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PriceTickerResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    PriceTickerResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        PriceTickerResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        Tickers &price(const std::int64_t value) SBE_NOEXCEPT
        {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PriceTickerResponse &_writer)
{
    PriceTickerResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "PriceTickerResponse", )";
