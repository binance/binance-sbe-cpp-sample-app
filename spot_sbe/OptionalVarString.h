...
#ifndef _SPOT_SBE_OPTIONALVARSTRING_CXX_H_
#define _SPOT_SBE_OPTIONALVARSTRING_CXX_H_
...
public:
    ...
    OptionalVarString(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        OptionalVarString(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    OptionalVarString(
        char *buffer,
        const std::uint64_t bufferLength) :
        OptionalVarString(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    OptionalVarString &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    OptionalVarString &length(const std::uint16_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, OptionalVarString &writer)
{
    builder << '{';
    builder << R"("length": )";
