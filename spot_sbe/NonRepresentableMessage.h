...
#ifndef _SPOT_SBE_NONREPRESENTABLEMESSAGE_CXX_H_
#define _SPOT_SBE_NONREPRESENTABLEMESSAGE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(999);
    ...
    NonRepresentableMessage() = default;

    NonRepresentableMessage(
        char *buffer,
    ...
    NonRepresentableMessage(char *buffer, const std::uint64_t bufferLength) :
        NonRepresentableMessage(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    NonRepresentableMessage(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        NonRepresentableMessage(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    NonRepresentableMessage &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    NonRepresentableMessage &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    NonRepresentableMessage &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    NonRepresentableMessage &sbeRewind()
    {
    ...
    {
        NonRepresentableMessage skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const NonRepresentableMessage &_writer)
{
    NonRepresentableMessage writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "NonRepresentableMessage", )";
