...
#ifndef _SPOT_SBE_LOTSIZEFILTER_CXX_H_
#define _SPOT_SBE_LOTSIZEFILTER_CXX_H_
...
public:
    ...
    LotSizeFilter(char *buffer, const std::uint64_t bufferLength) :
        LotSizeFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    LotSizeFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        LotSizeFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    LotSizeFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    LotSizeFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        LotSizeFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::LotSize;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::LotSize);
    }
    ...
    LotSizeFilter &qtyExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    LotSizeFilter &minQty(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    LotSizeFilter &maxQty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    LotSizeFilter &stepSize(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const LotSizeFilter &_writer)
{
    LotSizeFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
