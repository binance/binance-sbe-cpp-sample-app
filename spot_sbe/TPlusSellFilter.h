...
#ifndef _SPOT_SBE_TPLUSSELLFILTER_CXX_H_
#define _SPOT_SBE_TPLUSSELLFILTER_CXX_H_
...
public:
    ...
    TPlusSellFilter(char *buffer, const std::uint64_t bufferLength) :
        TPlusSellFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    TPlusSellFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        TPlusSellFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    TPlusSellFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    TPlusSellFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        TPlusSellFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::TPlusSell;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::TPlusSell);
    }

    SBE_NODISCARD static const char *endTimeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t endTimeId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t endTimeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool endTimeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t endTimeEncodingOffset() SBE_NOEXCEPT
    {
        return 0;
    }

    static SBE_CONSTEXPR std::int64_t endTimeNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t endTimeMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t endTimeMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t endTimeEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t endTime() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    TPlusSellFilter &endTime(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const TPlusSellFilter &_writer)
{
    TPlusSellFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "TPlusSellFilter", )";
