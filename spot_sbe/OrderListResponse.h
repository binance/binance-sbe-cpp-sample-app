...
#ifndef _SPOT_SBE_ORDERLISTRESPONSE_CXX_H_
#define _SPOT_SBE_ORDERLISTRESPONSE_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(19);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(313);
    ...
    OrderListResponse(char *buffer, const std::uint64_t bufferLength) :
        OrderListResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderListResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderListResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(313);
    }
    ...
    OrderListResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderListResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    OrderListResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        OrderListResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    OrderListResponse &orderListId(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    OrderListResponse &contingencyType(const ContingencyType::Value value) SBE_NOEXCEPT
    {
    ...
    OrderListResponse &listStatusType(const ListStatusType::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    OrderListResponse &listOrderStatus(const ListOrderStatus::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    OrderListResponse &transactionTime(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
public:
    ...
    OrderListResponse &putListClientOrderId(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
        std::uint8_t lengthFieldValue = (length);
    ...
    OrderListResponse &putListClientOrderId(const std::string &str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    OrderListResponse &putListClientOrderId(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    OrderListResponse &putSymbol(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
        std::uint8_t lengthFieldValue = (length);
    ...
    OrderListResponse &putSymbol(const std::string &str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    #if __cplusplus >= 201703L
    OrderListResponse &putSymbol(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderListResponse &_writer)
{
    OrderListResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "OrderListResponse", )";
