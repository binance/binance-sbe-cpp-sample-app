...
#ifndef _SPOT_SBE_OPTIONALMESSAGEDATA16_CXX_H_
#define _SPOT_SBE_OPTIONALMESSAGEDATA16_CXX_H_
...
public:
    ...
    OptionalMessageData16() = default;

    OptionalMessageData16(
        char *buffer,
    ...
    OptionalMessageData16(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        OptionalMessageData16(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    OptionalMessageData16(
        char *buffer,
        const std::uint64_t bufferLength) :
        OptionalMessageData16(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    OptionalMessageData16 &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    OptionalMessageData16 &length(const std::uint16_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, OptionalMessageData16 &writer)
{
    builder << '{';
    builder << R"("length": )";
