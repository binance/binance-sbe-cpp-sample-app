...
#ifndef _SPOT_SBE_PRICETICKERSYMBOLRESPONSE_CXX_H_
#define _SPOT_SBE_PRICETICKERSYMBOLRESPONSE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(209);
    ...
    PriceTickerSymbolResponse() = default;

    PriceTickerSymbolResponse(
        char *buffer,
    ...
    PriceTickerSymbolResponse(char *buffer, const std::uint64_t bufferLength) :
        PriceTickerSymbolResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    PriceTickerSymbolResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        PriceTickerSymbolResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(209);
    }
    ...
    PriceTickerSymbolResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PriceTickerSymbolResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    PriceTickerSymbolResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    PriceTickerSymbolResponse &sbeRewind()
    {
    ...
    {
        PriceTickerSymbolResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    PriceTickerSymbolResponse &priceExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    PriceTickerSymbolResponse &price(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    PriceTickerSymbolResponse &putSymbol(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    PriceTickerSymbolResponse &putSymbol(const std::string &str)
    {
        if (str.length() > 254)
        {
    ...
    #if __cplusplus >= 201703L
    PriceTickerSymbolResponse &putSymbol(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PriceTickerSymbolResponse &_writer)
{
    PriceTickerSymbolResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "PriceTickerSymbolResponse", )";
