...
#ifndef _SPOT_SBE_BLOCKTRADESRESPONSE_CXX_H_
#define _SPOT_SBE_BLOCKTRADESRESPONSE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(219);
    ...
    BlockTradesResponse(char *buffer, const std::uint64_t bufferLength) :
        BlockTradesResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    BlockTradesResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        BlockTradesResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    BlockTradesResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    BlockTradesResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    BlockTradesResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        BlockTradesResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    BlockTradesResponse &priceExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    BlockTradesResponse &qtyExponent(const std::int8_t value) SBE_NOEXCEPT
    {
        std::int8_t val = (value);
    ...
    #pragma GCC diagnostic pop
        ...
        BlockTrades &id(const std::int64_t value) SBE_NOEXCEPT
        {
        ...
        BlockTrades &price(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        BlockTrades &qty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        BlockTrades &quoteQty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        BlockTrades &time(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        BlockTrades &isBuyerMaker(const BoolEnum::Value value) SBE_NOEXCEPT
        {
        ...
        friend std::basic_ostream<CharT, Traits> & operator << (
            std::basic_ostream<CharT, Traits> &builder, BlockTrades &writer)
        {
            builder << '{';
            builder << R"("id": )";
            builder << +writer.id();
...
private:
    BlockTrades m_blockTrades;

public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t blockTradesId() SBE_NOEXCEPT
    {
        return 100;
    }

    SBE_NODISCARD inline BlockTrades &blockTrades()
    {
        m_blockTrades.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_blockTrades;
    }

    BlockTrades &blockTradesCount(const std::uint32_t count)
    {
        m_blockTrades.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_blockTrades;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t blockTradesSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool blockTradesInActingVersion() const SBE_NOEXCEPT
    {
        return true;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const BlockTradesResponse &_writer)
{
    BlockTradesResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "BlockTradesResponse", )";
    ...
    builder << ", ";
    {
        ...
        builder << R"("blockTrades": [)";
        writer.blockTrades().forEach(
...
void skip()
{
    auto &blockTradesGroup { blockTrades() };
    while (blockTradesGroup.hasNext())
    {
        blockTradesGroup.next().skip();
    }
}
...
SBE_NODISCARD static std::size_t computeLength(std::size_t blockTradesLength = 0)
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += BlockTrades::sbeHeaderSize();
    if (blockTradesLength > 2147483647LL)
    {
        throw std::runtime_error("blockTradesLength outside of allowed range [E110]");
    }
    length += blockTradesLength *BlockTrades::sbeBlockLength();
