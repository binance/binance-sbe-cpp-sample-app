...
#ifndef _SPOT_SBE_MAXNUMORDERAMENDSFILTER_CXX_H_
#define _SPOT_SBE_MAXNUMORDERAMENDSFILTER_CXX_H_
...
public:
    ...
    MaxNumOrderAmendsFilter() = default;
    ...
    MaxNumOrderAmendsFilter(char *buffer, const std::uint64_t bufferLength) :
        MaxNumOrderAmendsFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MaxNumOrderAmendsFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MaxNumOrderAmendsFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MaxNumOrderAmendsFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MaxNumOrderAmendsFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MaxNumOrderAmendsFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    MaxNumOrderAmendsFilter &sbeRewind()
    {
    ...
    {
        MaxNumOrderAmendsFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::MaxNumOrderAmends;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::MaxNumOrderAmends);
    }

    SBE_NODISCARD static const char *maxNumOrderAmendsMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t maxNumOrderAmendsId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t maxNumOrderAmendsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool maxNumOrderAmendsInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t maxNumOrderAmendsEncodingOffset() SBE_NOEXCEPT
    {
        return 0;
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrderAmendsNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrderAmendsMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrderAmendsMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t maxNumOrderAmendsEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t maxNumOrderAmends() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    MaxNumOrderAmendsFilter &maxNumOrderAmends(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MaxNumOrderAmendsFilter &_writer)
{
    MaxNumOrderAmendsFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MaxNumOrderAmendsFilter", )";
    ...
    builder << ", ";
    builder << R"("maxNumOrderAmends": )";
    builder << +writer.maxNumOrderAmends();
