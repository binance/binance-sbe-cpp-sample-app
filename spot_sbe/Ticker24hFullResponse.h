...
#ifndef _SPOT_SBE_TICKER24HFULLRESPONSE_CXX_H_
#define _SPOT_SBE_TICKER24HFULLRESPONSE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(206);
    ...
    Ticker24hFullResponse() = default;

    Ticker24hFullResponse(
        char *buffer,
    ...
    Ticker24hFullResponse(char *buffer, const std::uint64_t bufferLength) :
        Ticker24hFullResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Ticker24hFullResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Ticker24hFullResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Ticker24hFullResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Ticker24hFullResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    Ticker24hFullResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    Ticker24hFullResponse &sbeRewind()
    {
    ...
    {
        Ticker24hFullResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
            ...
            m_bufferLength = bufferLength;
            GroupSizeEncoding dimensions(buffer, *pos, bufferLength, actingVersion);
            dimensions.blockLength(static_cast<std::uint16_t>(182));
        ...
        Tickers &priceChange(const std::int64_t value) SBE_NOEXCEPT
        {
        ...
        Tickers &priceChangePercent(const float value) SBE_NOEXCEPT
        {
            union sbe_float_as_uint_u val;
            val.fp_value = value;
            val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_32(val.uint_value);
        ...
        Tickers &weightedAvgPrice(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Tickers &prevClosePrice(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Tickers &lastQty(const std::uint64_t index, const std::uint8_t value)
        {
            if (index >= 16)
            {
        ...
        #ifdef SBE_USE_SPAN
        SBE_NODISCARD std::span<const std::uint8_t> getLastQtyAsSpan() const SBE_NOEXCEPT
        {
        ...
        Tickers &putLastQty(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            static_assert(N <= 16, "array too large for putLastQty");

            std::memcpy(m_buffer + m_offset + 38, src.data(), sizeof(std::uint8_t) * N);
        ...
        #ifdef SBE_USE_SPAN
        Tickers &putLastQty(std::span<const std::uint8_t> src)
        {
            const std::size_t srcLength = src.size();
            if (srcLength > 16)
            {
                throw std::runtime_error("array too large for putLastQty [E106]");
            }

            std::memcpy(m_buffer + m_offset + 38, src.data(), sizeof(std::uint8_t) * srcLength);
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Tickers &putLastQty(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        Tickers &putLastQty(const char *const src) SBE_NOEXCEPT
        #endif
        {
        ...
        Tickers &bidPrice(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Tickers &bidQty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Tickers &askPrice(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Tickers &askQty(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        ...
        Tickers &putVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            ...
            std::memcpy(m_buffer + m_offset + 110, src.data(), sizeof(std::uint8_t) * N);
            for (std::size_t start = N; start < 16; ++start)
            {
        ...
        Tickers &putVolume(std::span<const std::uint8_t> src)
        {
            ...
            std::memcpy(m_buffer + m_offset + 110, src.data(), sizeof(std::uint8_t) * srcLength);
            for (std::size_t start = srcLength; start < 16; ++start)
            {
                m_buffer[m_offset + 110 + start] = 0;
            }
        ...
        {
            const char *buffer = m_buffer + m_offset + 126;
            return std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(buffer), 16);
        }
        #endif
        ...
        Tickers &putQuoteVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            ...
            std::memcpy(m_buffer + m_offset + 126, src.data(), sizeof(std::uint8_t) * N);
            for (std::size_t start = N; start < 16; ++start)
            {
                m_buffer[m_offset + 126 + start] = 0;
            }
        ...
        Tickers &putQuoteVolume(std::span<const std::uint8_t> src)
        {
            ...
            std::memcpy(m_buffer + m_offset + 126, src.data(), sizeof(std::uint8_t) * srcLength);
            for (std::size_t start = srcLength; start < 16; ++start)
            {
                m_buffer[m_offset + 126 + start] = 0;
            }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Ticker24hFullResponse &_writer)
{
    Ticker24hFullResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "Ticker24hFullResponse", )";
