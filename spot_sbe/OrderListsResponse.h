...
#ifndef _SPOT_SBE_ORDERLISTSRESPONSE_CXX_H_
#define _SPOT_SBE_ORDERLISTSRESPONSE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(314);
    ...
    OrderListsResponse(char *buffer, const std::uint64_t bufferLength) :
        OrderListsResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderListsResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderListsResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(314);
    }
    ...
    OrderListsResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderListsResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    OrderListsResponse &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        OrderListsResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        OrderLists &orderListId(const std::int64_t value) SBE_NOEXCEPT
        {
        ...
        OrderLists &contingencyType(const ContingencyType::Value value) SBE_NOEXCEPT
        {
        ...
        OrderLists &listStatusType(const ListStatusType::Value value) SBE_NOEXCEPT
        {
            std::uint8_t val = (value);
        ...
        OrderLists &listOrderStatus(const ListOrderStatus::Value value) SBE_NOEXCEPT
        {
            std::uint8_t val = (value);
        ...
        OrderLists &transactionTime(const std::int64_t value) SBE_NOEXCEPT
        {
            std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
public:
        ...
        OrderLists &putListClientOrderId(const char *src, const std::uint8_t length)
        {
            std::uint64_t lengthOfLengthField = 1;
            std::uint64_t lengthPosition = sbePosition();
            std::uint8_t lengthFieldValue = (length);
        ...
        OrderLists &putListClientOrderId(const std::string &str)
        {
            if (str.length() > 254)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        #if __cplusplus >= 201703L
        OrderLists &putListClientOrderId(const std::string_view str)
        {
            if (str.length() > 254)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        OrderLists &putSymbol(const char *src, const std::uint8_t length)
        {
            std::uint64_t lengthOfLengthField = 1;
            std::uint64_t lengthPosition = sbePosition();
            std::uint8_t lengthFieldValue = (length);
        ...
        OrderLists &putSymbol(const std::string &str)
        {
            if (str.length() > 254)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        #if __cplusplus >= 201703L
        OrderLists &putSymbol(const std::string_view str)
        {
            if (str.length() > 254)
            {
                throw std::runtime_error("std::string too long for length type [E109]");
        ...
        template<typename CharT, typename Traits>
        friend std::basic_ostream<CharT, Traits> & operator << (
            std::basic_ostream<CharT, Traits> &builder, OrderLists &writer)
        {
            builder << '{';
...
private:
    OrderLists m_orderLists;

public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t orderListsId() SBE_NOEXCEPT
    {
        return 100;
    }

    SBE_NODISCARD inline OrderLists &orderLists()
    {
        m_orderLists.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_orderLists;
    }

    OrderLists &orderListsCount(const std::uint32_t count)
    {
        m_orderLists.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_orderLists;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t orderListsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool orderListsInActingVersion() const SBE_NOEXCEPT
    {
        return true;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderListsResponse &_writer)
{
    OrderListsResponse writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "OrderListsResponse", )";
    ...
    {
        bool atLeastOne = false;
        builder << R"("orderLists": [)";
        writer.orderLists().forEach(
...
void skip()
{
    auto &orderListsGroup { orderLists() };
    while (orderListsGroup.hasNext())
    {
        orderListsGroup.next().skip();
    }
}
...
SBE_NODISCARD static std::size_t computeLength(const std::vector<std::tuple<
        const std::vector<std::tuple<
            ...
            std::size_t>> &,
        std::size_t,
        std::size_t>> &orderListsItemLengths = {})
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += OrderLists::sbeHeaderSize();
    if (orderListsItemLengths.size() > 2147483647LL)
    {
        throw std::runtime_error("orderListsItemLengths.size() outside of allowed range [E110]");
    }

    for (const auto &e: orderListsItemLengths)
    {
        #if __cplusplus >= 201703L
        length += std::apply(OrderLists::computeLength, e);
        #else
        length += OrderLists::computeLength(std::get<0>(e), std::get<1>(e), std::get<2>(e));
        #endif
    }
