...
#ifndef _SPOT_SBE_MESSAGEDATA16_CXX_H_
#define _SPOT_SBE_MESSAGEDATA16_CXX_H_
...
public:
    ...
    MessageData16(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        MessageData16(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    MessageData16(
        char *buffer,
        const std::uint64_t bufferLength) :
        MessageData16(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    MessageData16 &length(const std::uint16_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MessageData16 &writer)
{
    builder << '{';
    builder << R"("length": )";
