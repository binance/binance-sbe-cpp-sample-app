...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(203);
    ...
    {
        KlinesResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #pragma GCC diagnostic pop
        ...
        Klines &putVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            static_assert(N <= 16, "array too large for putVolume");

            std::memcpy(m_buffer + m_offset + 40, src.data(), sizeof(std::uint8_t) * N);
        ...
        #ifdef SBE_USE_SPAN
        Klines &putVolume(std::span<const std::uint8_t> src)
        {
            ...
            std::memcpy(m_buffer + m_offset + 40, src.data(), sizeof(std::uint8_t) * srcLength);
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Klines &putVolume(T&& src)  SBE_NOEXCEPT requires
        ...
        {
            const char *buffer = m_buffer + m_offset + 64;
            return std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(buffer), 16);
        }
        #endif
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Klines &putQuoteVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            static_assert(N <= 16, "array too large for putQuoteVolume");

            std::memcpy(m_buffer + m_offset + 64, src.data(), sizeof(std::uint8_t) * N);
            for (std::size_t start = N; start < 16; ++start)
            {
                m_buffer[m_offset + 64 + start] = 0;
            }
        ...
        #ifdef SBE_USE_SPAN
        Klines &putQuoteVolume(std::span<const std::uint8_t> src)
        {
            const std::size_t srcLength = src.size();
            if (srcLength > 16)
            {
            ...
            std::memcpy(m_buffer + m_offset + 64, src.data(), sizeof(std::uint8_t) * srcLength);
            for (std::size_t start = srcLength; start < 16; ++start)
            {
                m_buffer[m_offset + 64 + start] = 0;
            }
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Klines &putQuoteVolume(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        ...
        #ifdef SBE_USE_SPAN
        SBE_NODISCARD std::span<const std::uint8_t> getTakerBuyBaseVolumeAsSpan() const SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Klines &putTakerBuyBaseVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            static_assert(N <= 16, "array too large for putTakerBuyBaseVolume");

            std::memcpy(m_buffer + m_offset + 88, src.data(), sizeof(std::uint8_t) * N);
            for (std::size_t start = N; start < 16; ++start)
            {
        ...
        #ifdef SBE_USE_SPAN
        Klines &putTakerBuyBaseVolume(std::span<const std::uint8_t> src)
        {
            const std::size_t srcLength = src.size();
            if (srcLength > 16)
            {
                throw std::runtime_error("array too large for putTakerBuyBaseVolume [E106]");
            }

            std::memcpy(m_buffer + m_offset + 88, src.data(), sizeof(std::uint8_t) * srcLength);
            for (std::size_t start = srcLength; start < 16; ++start)
            {
                m_buffer[m_offset + 88 + start] = 0;
            }
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Klines &putTakerBuyBaseVolume(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        ...
        #ifdef SBE_USE_SPAN
        SBE_NODISCARD std::span<const std::uint8_t> getTakerBuyQuoteVolumeAsSpan() const SBE_NOEXCEPT
        {
            const char *buffer = m_buffer + m_offset + 104;
            return std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(buffer), 16);
        }
        #endif
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        Klines &putTakerBuyQuoteVolume(std::span<const std::uint8_t, N> src) SBE_NOEXCEPT
        {
            static_assert(N <= 16, "array too large for putTakerBuyQuoteVolume");

            std::memcpy(m_buffer + m_offset + 104, src.data(), sizeof(std::uint8_t) * N);
            for (std::size_t start = N; start < 16; ++start)
            {
                m_buffer[m_offset + 104 + start] = 0;
            }
        ...
        #ifdef SBE_USE_SPAN
        Klines &putTakerBuyQuoteVolume(std::span<const std::uint8_t> src)
        {
            const std::size_t srcLength = src.size();
            if (srcLength > 16)
            {
                throw std::runtime_error("array too large for putTakerBuyQuoteVolume [E106]");
            }

            std::memcpy(m_buffer + m_offset + 104, src.data(), sizeof(std::uint8_t) * srcLength);
            for (std::size_t start = srcLength; start < 16; ++start)
            {
                m_buffer[m_offset + 104 + start] = 0;
            }
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        Klines &putTakerBuyQuoteVolume(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
