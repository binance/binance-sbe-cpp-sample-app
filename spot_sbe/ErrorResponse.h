...
#ifndef _SPOT_SBE_ERRORRESPONSE_CXX_H_
#define _SPOT_SBE_ERRORRESPONSE_CXX_H_
...
public:
    ...
    ErrorResponse(char *buffer, const std::uint64_t bufferLength) :
        ErrorResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ErrorResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ErrorResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ErrorResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ErrorResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ErrorResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    static SBE_CONSTEXPR std::int16_t codeNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT16;
    }

    static SBE_CONSTEXPR std::int16_t codeMinValue() SBE_NOEXCEPT
    {
        return static_cast<std::int16_t>(-32767);
    }

    static SBE_CONSTEXPR std::int16_t codeMaxValue() SBE_NOEXCEPT
    {
        return static_cast<std::int16_t>(32767);
    }
    ...
    SBE_NODISCARD std::int16_t code() const SBE_NOEXCEPT
    {
        std::int16_t val;
    ...
    ErrorResponse &code(const std::int16_t value) SBE_NOEXCEPT
    {
    ...
    ErrorResponse &serverTime(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *retryAfterMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "optional";
    ...
    static SBE_CONSTEXPR std::uint16_t retryAfterId() SBE_NOEXCEPT
    {
        return 3;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t retryAfterSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool retryAfterInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t retryAfterEncodingOffset() SBE_NOEXCEPT
    {
        return 10;
    }

    static SBE_CONSTEXPR std::int64_t retryAfterNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t retryAfterMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t retryAfterMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t retryAfterEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t retryAfter() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ErrorResponse &retryAfter(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    static const char *msgCharacterEncoding() SBE_NOEXCEPT
    {
        return "UTF-8";
    }

    static SBE_CONSTEXPR std::uint64_t msgSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool msgInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }
    ...
    static SBE_CONSTEXPR std::uint64_t msgHeaderLength() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD std::uint16_t msgLength() const
    {
        std::uint16_t length;
    ...
    std::uint64_t getMsg(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ErrorResponse &putMsg(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getMsgAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getMsgAsString();
    ...
    #if __cplusplus >= 201703L
    std::string_view getMsgAsStringView()
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    ErrorResponse &putMsg(const std::string &str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putMsg(str.data(), static_cast<std::uint16_t>(str.length()));
    }

    #if __cplusplus >= 201703L
    ErrorResponse &putMsg(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
    ...
    ErrorResponse &putData(const char *src, const std::uint32_t length)
    {
        std::uint64_t lengthOfLengthField = 4;
        std::uint64_t lengthPosition = sbePosition();
    ...
    ErrorResponse &putData(const std::string &str)
    {
    ...
    #if __cplusplus >= 201703L
    ErrorResponse &putData(const std::string_view str)
    {
        if (str.length() > 2147483647)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ErrorResponse &_writer)
{
    ...
    builder << ", ";
    builder << R"("retryAfter": )";
    builder << +writer.retryAfter();

    builder << ", ";
    builder << R"("msg": )";
    builder << '"' <<
        writer.getMsgAsJsonEscapedString().c_str() << '"';
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += msgHeaderLength();
    if (msgLength > 65534LL)
    {
        throw std::runtime_error("msgLength too long for length type [E109]");
    }
    length += msgLength;

    length += dataHeaderLength();
    if (dataLength > 2147483647LL)
    {
