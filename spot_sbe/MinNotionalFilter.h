...
#ifndef _SPOT_SBE_MINNOTIONALFILTER_CXX_H_
#define _SPOT_SBE_MINNOTIONALFILTER_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(14);
    ...
    MinNotionalFilter(char *buffer, const std::uint64_t bufferLength) :
        MinNotionalFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MinNotionalFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MinNotionalFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MinNotionalFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MinNotionalFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MinNotionalFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        MinNotionalFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::MinNotional;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::MinNotional);
    }
    ...
    MinNotionalFilter &priceExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    MinNotionalFilter &minNotional(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *applyToMarketMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t applyToMarketId() SBE_NOEXCEPT
    {
        return 4;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t applyToMarketSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool applyToMarketInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t applyToMarketEncodingOffset() SBE_NOEXCEPT
    {
        return 9;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t applyToMarketEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t applyToMarketRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD BoolEnum::Value applyToMarket() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 9, sizeof(std::uint8_t));
    ...
    MinNotionalFilter &applyToMarket(const BoolEnum::Value value) SBE_NOEXCEPT
    {
    ...
    MinNotionalFilter &avgPriceMins(const std::int32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MinNotionalFilter &_writer)
{
    MinNotionalFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MinNotionalFilter", )";
    ...
    builder << ", ";
    builder << R"("applyToMarket": )";
    builder << '"' << writer.applyToMarket() << '"';
