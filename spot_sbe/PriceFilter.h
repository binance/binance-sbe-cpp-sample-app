...
#ifndef _SPOT_SBE_PRICEFILTER_CXX_H_
#define _SPOT_SBE_PRICEFILTER_CXX_H_
...
public:
    ...
    PriceFilter(char *buffer, const std::uint64_t bufferLength) :
        PriceFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    PriceFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        PriceFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    PriceFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PriceFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        PriceFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::PriceFilter;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::PriceFilter);
    }
    ...
    PriceFilter &priceExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    PriceFilter &minPrice(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    PriceFilter &maxPrice(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    PriceFilter &tickSize(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PriceFilter &_writer)
{
    PriceFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
