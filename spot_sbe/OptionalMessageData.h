...
#ifndef _SPOT_SBE_OPTIONALMESSAGEDATA_CXX_H_
#define _SPOT_SBE_OPTIONALMESSAGEDATA_CXX_H_
...
public:
    ...
    OptionalMessageData(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        OptionalMessageData(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    OptionalMessageData(
        char *buffer,
        const std::uint64_t bufferLength) :
        OptionalMessageData(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    OptionalMessageData &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    OptionalMessageData &length(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, OptionalMessageData &writer)
{
    builder << '{';
    builder << R"("length": )";
