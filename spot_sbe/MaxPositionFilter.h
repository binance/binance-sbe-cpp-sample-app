...
#ifndef _SPOT_SBE_MAXPOSITIONFILTER_CXX_H_
#define _SPOT_SBE_MAXPOSITIONFILTER_CXX_H_
...
public:
    ...
    MaxPositionFilter(char *buffer, const std::uint64_t bufferLength) :
        MaxPositionFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MaxPositionFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MaxPositionFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MaxPositionFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MaxPositionFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MaxPositionFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        MaxPositionFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::MaxPosition;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::MaxPosition);
    }
    ...
    MaxPositionFilter &qtyExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *maxPositionMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t maxPositionId() SBE_NOEXCEPT
    {
        return 3;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t maxPositionSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool maxPositionInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t maxPositionEncodingOffset() SBE_NOEXCEPT
    {
        return 1;
    }

    static SBE_CONSTEXPR std::int64_t maxPositionNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t maxPositionMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t maxPositionMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t maxPositionEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t maxPosition() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    MaxPositionFilter &maxPosition(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MaxPositionFilter &_writer)
{
    MaxPositionFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MaxPositionFilter", )";
    ...
    builder << ", ";
    builder << R"("maxPosition": )";
    builder << +writer.maxPosition();
