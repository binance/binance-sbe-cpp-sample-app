...
#ifndef _SPOT_SBE_MESSAGEDATA_CXX_H_
#define _SPOT_SBE_MESSAGEDATA_CXX_H_
...
public:
    ...
    MessageData(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        MessageData(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    MessageData(
        char *buffer,
        const std::uint64_t bufferLength) :
        MessageData(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    MessageData &length(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MessageData &writer)
{
    builder << '{';
    builder << R"("length": )";
