...
#ifndef _SPOT_SBE_ALLOWEDSELFTRADEPREVENTIONMODES_CXX_H_
#define _SPOT_SBE_ALLOWEDSELFTRADEPREVENTIONMODES_CXX_H_
...
public:
    ...
    AllowedSelfTradePreventionModes() = default;
    ...
    AllowedSelfTradePreventionModes(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        AllowedSelfTradePreventionModes(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    AllowedSelfTradePreventionModes(
        char *buffer,
        const std::uint64_t bufferLength) :
        AllowedSelfTradePreventionModes(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    AllowedSelfTradePreventionModes &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    AllowedSelfTradePreventionModes &rawValue(std::uint8_t value)
    {
        std::memcpy(m_buffer + m_offset, &value, sizeof(std::uint8_t));
        return *this;
    }

    static bool none(const std::uint8_t bits)
    {
        return (bits & (static_cast<std::uint8_t>(1) << 0u)) != 0;
    }

    static std::uint8_t none(const std::uint8_t bits, const bool value)
    {
    ...
    AllowedSelfTradePreventionModes &none(const bool value)
    {
        std::uint8_t bits;
    ...
    static bool expireTaker(const std::uint8_t bits)
    {
        return (bits & (static_cast<std::uint8_t>(1) << 1u)) != 0;
    }

    static std::uint8_t expireTaker(const std::uint8_t bits, const bool value)
    {
    ...
    SBE_NODISCARD bool expireTaker() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset, sizeof(std::uint8_t));
    ...
    AllowedSelfTradePreventionModes &expireTaker(const bool value)
    {
        std::uint8_t bits;
        std::memcpy(&bits, m_buffer + m_offset, sizeof(std::uint8_t));
    ...
    static bool expireMaker(const std::uint8_t bits)
    {
        return (bits & (static_cast<std::uint8_t>(1) << 2u)) != 0;
    }

    static std::uint8_t expireMaker(const std::uint8_t bits, const bool value)
    {
    ...
    SBE_NODISCARD bool expireMaker() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset, sizeof(std::uint8_t));
    ...
    AllowedSelfTradePreventionModes &expireMaker(const bool value)
    {
        std::uint8_t bits;
        std::memcpy(&bits, m_buffer + m_offset, sizeof(std::uint8_t));
    ...
    static bool expireBoth(const std::uint8_t bits)
    {
        return (bits & (static_cast<std::uint8_t>(1) << 3u)) != 0;
    }

    static std::uint8_t expireBoth(const std::uint8_t bits, const bool value)
    {
    ...
    SBE_NODISCARD bool expireBoth() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset, sizeof(std::uint8_t));
    ...
    AllowedSelfTradePreventionModes &expireBoth(const bool value)
    {
        std::uint8_t bits;
        std::memcpy(&bits, m_buffer + m_offset, sizeof(std::uint8_t));
        ...
        std::basic_ostream<CharT, Traits> &builder, AllowedSelfTradePreventionModes &writer)
    {
        ...
        if (writer.expireTaker())
        {
            if (atLeastOne)
            {
                builder << ",";
            }
            builder << R"("expireTaker")";
            atLeastOne = true;
        }
        if (writer.expireMaker())
        {
            if (atLeastOne)
            {
                builder << ",";
            }
            builder << R"("expireMaker")";
            atLeastOne = true;
        }
        if (writer.expireBoth())
        {
            if (atLeastOne)
            {
                builder << ",";
