...
#ifndef _SPOT_SBE_EXCHANGEMAXNUMORDERSFILTER_CXX_H_
#define _SPOT_SBE_EXCHANGEMAXNUMORDERSFILTER_CXX_H_
...
public:
    ...
    ExchangeMaxNumOrdersFilter() = default;
    ...
    ExchangeMaxNumOrdersFilter(char *buffer, const std::uint64_t bufferLength) :
        ExchangeMaxNumOrdersFilter(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ExchangeMaxNumOrdersFilter(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ExchangeMaxNumOrdersFilter(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ExchangeMaxNumOrdersFilter &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ExchangeMaxNumOrdersFilter &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ExchangeMaxNumOrdersFilter &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    ExchangeMaxNumOrdersFilter &sbeRewind()
    {
    ...
    {
        ExchangeMaxNumOrdersFilter skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    {
        return FilterType::Value::ExchangeMaxNumOrders;
    }
    ...
    {
        return static_cast<std::uint8_t>(FilterType::Value::ExchangeMaxNumOrders);
    }

    SBE_NODISCARD static const char *maxNumOrdersMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t maxNumOrdersId() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t maxNumOrdersSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool maxNumOrdersInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t maxNumOrdersEncodingOffset() SBE_NOEXCEPT
    {
        return 0;
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrdersNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrdersMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t maxNumOrdersMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t maxNumOrdersEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t maxNumOrders() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    ExchangeMaxNumOrdersFilter &maxNumOrders(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ExchangeMaxNumOrdersFilter &_writer)
{
    ExchangeMaxNumOrdersFilter writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ExchangeMaxNumOrdersFilter", )";
    ...
    builder << ", ";
    builder << R"("maxNumOrders": )";
    builder << +writer.maxNumOrders();
