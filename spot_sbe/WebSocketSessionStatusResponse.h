...
public:
    ...
    {
        WebSocketSessionStatusResponse skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    WebSocketSessionStatusResponse &userDataStream(const BoolEnum::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    WebSocketSessionStatusResponse &putLoggedOnApiKey(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    WebSocketSessionStatusResponse &putLoggedOnApiKey(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #if __cplusplus >= 201703L
    WebSocketSessionStatusResponse &putLoggedOnApiKey(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
